...
def check_pwfs_pupils(valid_pixel_map, wfs__, n_it=3, correct=False):
    ...
    if correct is False:
        for i in range(4):
            I = wfs__.grabFullQuadrant(i + 1, valid_pixel_map)
            xc = I.shape[0] // 2
            [x, y] = np.asarray(centroid(I, threshold=0.3))
            I_ = np.abs(wfs__.grabFullQuadrant(i + 1))
            I_ /= I_.max()
        ...
        for i_it in range(n_it):
            wfs__.apply_shift_wfs(sx=xs, sy=ys)
...
utc_acquisition = "utc_2026-01-26_13-20-41"

dirc = config.paths.data_dir

path_to_experimental_data = dirc / "closed_loop" / (utc_acquisition + "_closed_loop.h5")

with h5py.File(path_to_experimental_data) as f:
    ...
    interaction_matrix_exp_cube = interaction_matrix_grp["interaction_matrix"][...]
    strokes_exp = interaction_matrix_grp["interaction_matrix"].attrs["strokes"]
    valid_pixels = reconstructor_grp["valid_pixels"][...]
...
# %% other saving convention

# with h5py.File(path_to_experimental_data) as f:
#     interaction_matrix_exp_cube = f["interaction_matrix"]["interaction_matrix"][...]
#     strokes_exp = f["interaction_matrix"]["interaction_matrix"].attrs["strokes"]
#     valid_pixels = f["reconstructor"]["valid_pixels"][...]
#     modal_basis = modal_basis_factor * f["interaction_matrix"]["modal_basis"][...]
#     turbulence = f["turbulence"]["turbulence"][...]
#     wfs_frames_exp = f["wfs_frames"][...]
#     total_exp = f["total"][...]
#     residual_exp = f["residual"][...]
#     frequency = f["turbulence"]["turbulence"].attrs["frequency"]
#     r0 = f["turbulence"]["turbulence"].attrs["r0"]
#     loop_gain = f.attrs["gain"][()]

# %% check modal basis std

pupil = get_circular_pupil(modal_basis.shape[1])
print(modal_basis[:, ~pupil].sum())

print((modal_basis[:, pupil].std(axis=1)))

# %% Extract experimental interaction matrix

interaction_matrix_exp = interaction_matrix_exp_cube[:, valid_pixels].T

# %% Extract simualtions parameters

telescope_resolution = modal_basis.shape[1]
wfs_resolution = valid_pixels.sum(axis=0).max() // 2
...
# assuming zero extra pixels for the bio-edge (no need), cropping of the input valid pixel maps
n_extra_0 = valid_pixels.shape[0] - 4 * wfs_resolution
n_extra_1 = valid_pixels.shape[1] - 4 * wfs_resolution

if n_extra_0 > 0:
    valid_pixels_sim = valid_pixels[
        n_extra_0 // 2 : n_extra_0 // 2, n_extra_1 // 2 : n_extra_1 // 2
    ]
    valid_pixels_sim = valid_pixels_sim.astype(bool)
else:
    valid_pixels_sim = np.pad(
        valid_pixels,
        [
            [-n_extra_0 // 2 + 1, -n_extra_0 // 2],
            [-n_extra_1 // 2 + 1, -n_extra_1 // 2],
        ],
    )
    valid_pixels_sim = valid_pixels_sim.astype(bool)
...
check_pwfs_pupils(valid_pixels_sim, wfs, correct=True, n_it=2)
...
pyramid = Pyramid(
    ...
    lightRatio=0.3,  # no need to use it since we input the valid pixel map
    n_pix_edge=0,
    n_pix_separation=0,
    postProcessing="fullFrame_sum_flux",  # normalisation based on the flux on the detector (same as what is done on the bench)
...
stroke_rad = strokes_exp.min()  # [rad]
...
interaction_matrix_pyramid = calib_pyramid.D * src.wavelength / (2 * np.pi)

reconstructor_pyramid = np.linalg.pinv(calib_pyramid.D)
...
for i, mode in enumerate(modes):
    ...
    axs[i, 1].axis("off")
    cbar_exp = fig.colorbar(im_exp, ax=axs[i, 1], fraction=0.046, pad=0.02)
    cbar_exp.formatter = ScalarFormatter(useMathText=True)
    cbar_exp.formatter.set_powerlimits((-2, 2))  # forces scientific notation
    cbar_exp.update_ticks()
    ...
    axs[i, 2].axis("off")
    cbar_sim = fig.colorbar(im_sim, ax=axs[i, 2], fraction=0.046, pad=0.02)
    cbar_sim.formatter = ScalarFormatter(useMathText=True)
    cbar_sim.formatter.set_powerlimits((-2, 2))
    cbar_sim.update_ticks()

# plt.savefig("interaction_matrix_visual.pdf", bbox_inches="tight", pad_inches=0.01)
plt.show()
...
reference_signal = (
    wfs.referenceSignal
)  # no experimental reference intensities at the time

# %%

sim_sensitivities = (
    np.diag(
        (interaction_matrix_sim / reference_signal.reshape((-1, 1)) ** 0.5).T
        @ (interaction_matrix_sim / reference_signal.reshape((-1, 1)) ** 0.5)
    )
    ** 0.5
)
...
exp_sensitivity = (
    np.diag(
        (interaction_matrix_exp / reference_signal.reshape((-1, 1)) ** 0.5).T
        @ (interaction_matrix_exp / reference_signal.reshape((-1, 1)) ** 0.5)
    )
    ** 0.5
)

plt.figure()
plt.plot(sim_sensitivities, label="sim_sensitivities")
plt.plot(exp_sensitivity, label="exp_sensitivity")
plt.plot(
    2**0.5 * np.ones(sim_sensitivities.size),
    label="sqrt(2) (ideal Grey Bi-O-Edge HO sensitivity)",
)

plt.plot(
    2**0.5 / 2 * np.ones(sim_sensitivities.size),
    label="sqrt(2)/2 (ideal Modulated Pyramid HO sensitivity)",
)

plt.plot()
plt.legend()
plt.title(utc_acquisition + "\nPhoton noise sensitivities")
plt.xlabel("# KL mode")
...
closed_loop_sim = close_the_loop(
    ...
    loop_gain=loop_gain,
    reference_intensities=reference_signal,
    display=False,
)
